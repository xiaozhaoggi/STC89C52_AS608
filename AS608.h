...
void Add_FR(void)
{
	...
	while(1)
	{
		switch (processnum)
		{
			...
			case 2:
				WRCommand(0x01);
				ShowQQChar(0x80,"对比两次指纹",6);
				ensure=PS_Match();
				if(ensure==0x00) 
				{	WRCommand(0x01);
					ShowQQChar(0x80,"对比成功",4);
					processnum=3;//跳到第四步
				}
				else
				{					
					WRCommand(0x01);
					ShowQQChar(0x80,"对比失败",4);
			...
			case 3:
				WRCommand(0x01);
				ShowQQChar(0x80,"生成指纹模板",6);
				Delay_Ms(300);
				ensure=PS_RegModel();
				if(ensure==0x00) 
				{
					WRCommand(0x01);
					ShowQQChar(0x80,"生成指纹模板成功",8);
					processnum=4;//跳到第五步
				}else {processnum=0;}
				Delay_Ms(500);
				break;
			...
			case 4:	
				WRCommand(0x01);
				ShowQQChar(0x80,"按14加按15减",7);
				ShowQQChar(0x90,"按16保存",4);
			  ...
			  ID_NUM=0;
				...
				{		
					WRCommand(0x01);;
					ShowQQChar(0x80,"录入指纹成功",6);
					Delay_Ms(300);
					ShowQQChar(0x80,"指纹模块测试程序",8);
					ShowQQChar(0x90,"11键添加指纹",6);
					ShowQQChar(0x88,"12键删除指纹",6);
					ShowQQChar(0x98,"13键验证指纹",6);
					return ;
				}else {/*WRCommand(0x01)*/;
							processnum=0;}					
				break;				
...
void Del_FR(void)
{
	...
	uint ID_NUM=0;
	unsigned char s[5];
	ShowQQChar(0x80,"14加15减16确认",6);
	ShowQQChar(0x90,"12清空指纹库",6);
	ShowQQChar(0x88,"11返回",3);
	while(key_num!=10)
	{
		key_num=KeyDown();
		if(key_num==14)
